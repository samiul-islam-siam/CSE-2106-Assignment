...
main    PROC
        ...
        ; ======================================================================
        LDR     R0, =SENSOR_HR
        MOV     R1, #125                ; > 120 ? triggers HR alert
        STRB    R1, [R0]
        LDR     R0, =SENSOR_O2
        MOV     R1, #88                 ; < 92 ? triggers O2 alert
        STRB    R1, [R0]
        LDR     R0, =SENSOR_SBP
        MOV     R1, #135                ; Normal (90-160)
        STRB    R1, [R0]
        LDR     R0, =SENSOR_DBP
        MOV     R1, #85
        STRB    R1, [R0]
        ...
        ; ======================================================================
        LDR     R0, =patient_array
        BL      check_vital_thresholds  ; Should create 2 alerts (HR + O2)
        ...
        MOVW    R10, #PATIENT_SIZE
        ADD     R0, R0, R10
        BL      check_vital_thresholds  ; Should create 0 alerts
        ...
        ; ======================================================================
        LDR     R0, =SENSOR_HR
        MOV     R1, #165                ; > 120 ? triggers HR alert
        STRB    R1, [R0]
        LDR     R0, =SENSOR_O2
        MOV     R1, #85                 ; < 92 ? triggers O2 alert
        STRB    R1, [R0]
        LDR     R0, =SENSOR_SBP
        MOV     R1, #170                ; > 160 ? triggers BP alert
        STRB    R1, [R0]
        LDR     R0, =SENSOR_DBP
        MOV     R1, #95
        STRB    R1, [R0]
        ...
        LSL     R10, R10, #1
        ADD     R0, R0, R10
        BL      check_vital_thresholds  ; Should create 3 alerts (HR+O2+BP)
